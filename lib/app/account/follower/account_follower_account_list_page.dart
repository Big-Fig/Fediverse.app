...
class AccountFollowerAccountListPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(appBar: FediSubPageTitleAppBar(
      title: tr("app.account.follower.title", args: [account.acct],),),
      body: SafeArea(child: Column(
        children: [Padding(padding: FediPadding.allBigPadding, child: Text(
        "app.account.list.privacy".tr(), textAlign: TextAlign.center,
        style: FediTextStyles.mediumShortBoldGrey,),),),
      Expanded(child: AccountPaginationListWidget(
...
void goToAccountFollowerAccountListPage(BuildContext context,
    IAccount account) {
  Navigator.push(context, MaterialPageRoute(builder: (context) {
    return AccountFollowerAccountCachedListBloc.provideToContext(
      context, account: account,
      child: AccountCachedPaginationBloc.provideToContext(context,
        child: AccountPaginationListBloc.provideToContext(context,
          child: AccountFollowerAccountListPage(account: account,),),),);
  }),);
}
