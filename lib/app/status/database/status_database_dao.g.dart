...
mixin _$StatusDaoMixin on DatabaseAccessor<AppDatabase> {
  ...
  DbStatus _rowToDbStatus(QueryRow row) {
    return DbStatus(
      ...
      mediaAttachments: $DbStatusesTable.$converter3
          .mapToDart(row.readString('media_attachments')),
      mentions:
          $DbStatusesTable.$converter4.mapToDart(row.readString('mentions')),
      tags: $DbStatusesTable.$converter5.mapToDart(row.readString('tags')),
      emojis: $DbStatusesTable.$converter6.mapToDart(row.readString('emojis')),
      poll: $DbStatusesTable.$converter7.mapToDart(row.readString('poll')),
      card: $DbStatusesTable.$converter8.mapToDart(row.readString('card')),
